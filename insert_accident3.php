...
// Inserting alert details
$return_val = "";
// Prepared statement, stage 1: prepare
if (!($stmt = $conn->prepare("CALL accident_alert_proc(?, @rval)"))) {
   echo "Prepare failed: (" . $conn->errno . ") " . $conn->error;
}

$stmt -> bind_param('s', $newrdno);

if (!$stmt->execute()) {
    echo "Execute failed: (" . $stmt->errno . ") " . $stmt->error;
}

// execute the second query to get procedure output
$result = $conn->query("SELECT @rval AS rval");
while($row = $result->fetch_assoc()) {
	$returnval = $row["rval"];
	echo "procedure returned $returnval\n";
}
